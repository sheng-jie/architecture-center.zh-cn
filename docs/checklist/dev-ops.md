...
---
# <a name="devops-checklist"></a>DevOps 清单

DevOps 是指将开发、质量保证和 IT 运营集成到统一的环境以及软件交付过程中。 评估 DevOps 的环境和过程时，可从此清单着手。
...
**阐述操作。** 以等同于产品代码的质量阐述所有工具、过程和自动化任务的运行方法。 阐述所支持的任何系统的当前设计和体系结构，以及恢复过程和其他维护流程。 专注于实际执行的步骤，而不是理论最佳的过程。 定期评审并更新文档。 对于代码，请确保包含有意义的注释，尤其是在公共 API 中；尽量使用工具自动构建代码文档。 

**分享知识。** 文档只对知道它存在并可以找到它的人员才有作用。 请确保文档组织有序且能轻松找到。 创新方式：使用牛皮纸袋（非正式演示）、视频或新闻稿分享知识。

## <a name="development"></a>开发

**为开发人员提供类似于生产的环境。** 如果开发和测试环境与生产环境不相符，则很难测试和诊断问题。 因此，应该让开发和测试环境尽量接近生产环境。 确保测试数据与生产环境中使用的数据一致，即使测试数据是示例数据而不是实际生产数据（出于隐私或符合性原因）。 计划构建并匿名化示例测试数据。

**确保所有已获授权的团队成员可以预配基础结构和部署应用程序。** 设置类似于生产环境中所用的资源和部署应用程序不应涉及复杂的手动任务，或者要求对系统有深层的技术知识。 任何具有适当权限的人应该能够创建或部署类似于生产环境中所用的资源，而无需求助于运营团队。 
...
**自动测试。** 手动测试软件的过程非常繁琐且容易出错。 请将常见的测试任务自动化，并将测试集成到构建过程中。 自动测试可确保测试覆盖范围一致，且能重现问题。 此外，应通过自动化工具执行集成式 UI 测试。 Azure 提供开发和测试资源，可帮助配置和执行测试。 有关详细信息，请参阅[开发和测试][dev-test]。

**故障测试。** 如果系统无法连接到服务，它会如何做出响应？ 服务再次可用后，系统能否恢复？ 将故障注入测试规定为测试和预演环境评审中的标准组成部分。 如果测试过程和做法比较健全，请考虑在生产环境中运行这些测试。 
...
**执行自动安全渗透测试。** 确保应用程序的安全与测试其他任何功能一样重要。 将自动渗透测试规定为构建和部署过程的标准组成部分。 针对已部署的应用程序计划定期安全测试和漏洞扫描，以监视打开的端口、终结点和攻击活动。 自动测试并不意味着无需定期展开深入的安全评审。

**执行自动业务连续性测试。** 针对大规模业务连续性方案（包括备份、恢复和故障转移）制定测试方法。 设置自动化过程来定期执行这些测试。

## <a name="release"></a>发布

**自动部署。** 将应用程序自动部署到测试、预演和生产环境。 自动化能够更快、更可靠地完成部署，并确保以一致的方式部署到任何受支持的环境。 它可以消除手动部署造成人为错误的风险。 此外，使用自动化还能将发布活动安排在方便的时间，以尽量降低潜在停机造成的影响。

**使用持续集成。** 持续集成 (CI) 是根据定期计划将所有开发人员代码合并到中心代码库，然后自动执行标准构建和测试过程的做法。 CI 可确保整个团队同时处理代码库且不发生冲突。 此外，它可以确保尽早发现代码缺陷。 最好是每次提交或签入代码时都要运行 CI 过程。 最起码每天应运行一次。

> 考虑采用[基于树干的开发模型][trunk-based]。 在此模型中，开发人员可将内容提交到单个树枝（树干）。 有一项要求是，提交内容不得破坏构建。 此模型有助于实现 CI，因为所有的功能性工作都在树干中完成，并且所有合并冲突都可在提交时解决。

**考虑使用持续交付。** 持续交付 (CD) 是通过自动构建、测试代码并将其部署到类似生产的环境，确保代码随时可供部署的做法。 添加持续交付来创建完整 CI/CD 管道的做法有助于尽早检测代码缺陷，并确保能够在极短的时间内发布已经过适当测试的更新。
...
**控制更新可用。** 确保您能够控制最终用户何时可以看到更新。 考虑使用功能切换来控制何时为终端用户启用功能。
...
**考虑使基础结构保持不可变。** 不可变基础结构是指在将基础结构部署到生产环境后不应对其进行修改的原则。 否则，可能会陷入这种状态：应用临时更改后，很难确切地知道哪些内容已更改。 不可变基础结构的工作原理是将整个服务器作为任何新部署的一部分进行替换。 这样，就能以一个块的形式测试和部署代码与宿主环境。 部署后，基础结构组件在下一个构建和部署周期之前不会修改。 
...
**在变更管理系统中管理所有资源。** 应在管理和版本控制系统中包含 DevOps 过程的方方面面，以便可以轻松跟踪和审核更改。 这包括代码、基础结构、配置、文档和脚本。 在整个测试/构建/评审过程中，应将所有这些类型的资源视为代码。 

**使用清单。** 创建操作清单，以确保遵循过程。 较大的手册中很容易遗漏内容，遵循清单可以促使阅读者关注可能被忽略的细节。 维护清单，并持续寻求可自动执行任务和优化过程的方法。
