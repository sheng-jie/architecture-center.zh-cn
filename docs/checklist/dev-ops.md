...
DevOps 是指将开发、质量保证和 IT 运营集成到统一的环境以及软件交付过程中。 

评估 DevOps 的环境和过程时，可从此查检表着手。
...
**优化速度并收集数据。** 规划的每种改进方案都是基于假设。 请尽量以最慢的步调行事。 将新的思路视为试验。 检测试验，以便可以收集生产数据来评估这些试验的有效性。 如果假设有误，应准备好尽快从失败中恢复。
...
**控制更改内容透露。** 确保控制向最终用户显示更新的时间。 考虑使用功能切换来控制何时为最终用户启用功能。
