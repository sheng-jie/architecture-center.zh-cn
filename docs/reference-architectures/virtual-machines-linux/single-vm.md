...
此体系结构显示有关在 Azure 中运行 Linux VM 的基准建议。 但是，不建议针对任务关键型工作负荷使用单个 VM，因为这会产生单一故障点。 为了提高可用性，请部署两个或更多个负载均衡的 VM。 有关详细信息，请参阅 [Running multiple VMs on Azure][multi-vm]（在 Azure 上运行多个 VM）。
